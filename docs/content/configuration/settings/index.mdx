...
```yaml
connection:
  providerJoinRestrictionTtl: "30d"
```
  </Tab>
  <Tab value="JSON">
```json
...
`connection.sessions` controls live-session behavior and optional reconnect recognition.
...
If one provider needs different behavior, set `overrides.sessionConcurrencyPolicy` for that provider in `providers.yml`.
...
Use a value comfortably longer than transient replication delays and scheduler jitter.
If it is too small, an online player can temporarily disappear from replicated session lookups before the next keepalive refresh rewrites the cache entry.
...
### Connection Scenarios Authentication

These settings affect the authentication pipeline.
Configure them under `connection.scenarios.authentication`.
...
- Takes: `true` or `false`
- Current default: `false`
- Affects: whether resumed authentication flows may continue while a provider join restriction is active

If this is `true`, a player who is resuming an already-pending authentication flow may continue even when the provider they are using is currently restricted.
This does not allow new authentication attempts through the restriction. It only affects resumed flows.
...
### Connection Scenarios Registration

These settings affect the registration pipeline.
Configure them under `connection.scenarios.registration`.
...
- Takes: `true` or `false`
- Current default: `false`
- Affects: whether resumed registration flows may continue while a provider join restriction is active

If this is `true`, a player who is resuming an already-pending registration flow may continue even when the provider they are using is currently restricted.
This does not allow new registration attempts through the restriction. It only affects resumed flows.
...
<Tabs items={["SEAMLESS", "INTERACTIVE"]}>
  ...
  <Tab value="INTERACTIVE">
    ...
    Use case: the player chooses a provider, enters a credential, confirms a prompt, or otherwise participates directly in the registration flow.
...
### Connection Scenarios Migration

These settings affect the migration pipeline.
Configure them under `connection.scenarios.migration`.
