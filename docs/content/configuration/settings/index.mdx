...
Recognition is a separate optional shortcut that can let a reconnecting player skip full provider authentication when the configured signals match a stored recognition snapshot.
...
#### recognition.snapshotTtl

<Badge type="warning">Advanced</Badge>

- Takes: a positive duration
- Current default: `"12h"`
- Affects: how long a saved recognition snapshot survives for reconnect recognition checks

This TTL belongs to recognition, not to the live-session lifetime.
If it expires, the player simply goes through normal provider authentication on the next reconnect.
