...
- Current template default: `"RANDOM"`
...
These scenario entries override `routing.defaults` for the matching pipeline.
...
- Takes: a map of provider id to positive duration
- Affects: provider-specific session TTL overrides

The key should be the provider id, such as `premium`.
...
- Current template default: `"REPLACE_EXISTING"`
...
- Current template default: `"SEAMLESS"`
...
- Current template default: `"PREFER"`
...
- Takes: `"SEAMLESS"` or `"INTERACTIVE"`
- Current template default: `"INTERACTIVE"`
...
<Tabs items={["SEAMLESS", "INTERACTIVE"]}>
  ...
  <Tab value="INTERACTIVE">
    ...
    The current template prefers an interactive migration flow.
...
Use the exact Velocity event class name as the key.
Missing entries are still registered with the built-in defaults, so this map is mainly for overrides and explicit opt-outs.
  </ContentTab>
</ContentTabs>
