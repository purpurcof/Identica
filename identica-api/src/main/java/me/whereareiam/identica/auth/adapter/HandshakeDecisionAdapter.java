...
public abstract class HandshakeDecisionAdapter {
	...
	public final void apply(
		...
		switch (decision.getStatus()) {
			case FORCE_ONLINE -> target.forceOnline();
			case FORCE_OFFLINE -> target.forceOffline();
			case DENY -> target.deny(Serializer.serialize(resolveHandshakeMessage(decision.getMessage())));
			default -> {
			}
		}
	}
	...
	public interface HandshakeDecisionTarget {
		/**
		 * Forces online mode for this connection.
		 */
		void forceOnline();

		/**
		 * Forces offline mode for this connection.
		 */
		void forceOffline();
