...
public abstract class AbstractUsernamePhase {
	...
	protected boolean synchronize(
		...
		if (!isAuthoritativeProvider(link.getProviderId())) return false;
	...
	private boolean isAuthoritativeProvider(@Nullable String providerId) {
		...
		return providerManager.getProviders().stream()
				...
				.anyMatch(descriptor -> descriptor.getId().equalsIgnoreCase(providerId)
						&& descriptor.hasCapability(AuthoritativeUsernameCapability.CAPABILITY));
	}
}
