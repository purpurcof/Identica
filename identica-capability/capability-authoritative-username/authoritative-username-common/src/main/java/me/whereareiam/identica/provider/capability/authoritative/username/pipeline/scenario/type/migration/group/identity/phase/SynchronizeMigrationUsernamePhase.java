package me.whereareiam.identica.provider.capability.authoritative.username.pipeline.scenario.type.migration.group.identity.phase;
...
import me.whereareiam.identica.provider.capability.authoritative.username.event.UsernameChangedEvent;
import me.whereareiam.identica.provider.capability.authoritative.username.model.account.AccountUsernameHistoryEntry;
import me.whereareiam.identica.provider.capability.authoritative.username.pipeline.phase.base.AbstractUsernamePhase;
...
public class SynchronizeMigrationUsernamePhase extends AbstractUsernamePhase implements PipelinePhase<IdentityState> {
	...
	public @NotNull CompletionStage<PhaseResult<IdentityState>> execute(@NotNull PipelineState pipelineState, @NotNull IdentityState state) {
		...
		if (state.getContext() == null || state.getAccount() == null || state.getLink() == null || state.getProfile() == null) {
			state.setResult(PipelineResult.failed(String.join("\n", messagesProvider.get().getScenarios().getMigration().getErrors().getIdentity().getReplicationMissing())));
			return CompletableFuture.completedFuture(PhaseResult.pass(state));
		}
