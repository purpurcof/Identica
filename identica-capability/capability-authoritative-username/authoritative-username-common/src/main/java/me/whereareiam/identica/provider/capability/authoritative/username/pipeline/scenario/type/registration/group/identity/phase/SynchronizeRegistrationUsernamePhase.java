package me.whereareiam.identica.provider.capability.authoritative.username.pipeline.scenario.type.registration.group.identity.phase;
...
import me.whereareiam.identica.provider.capability.authoritative.username.pipeline.phase.base.AbstractSynchronizeUsernamePhase;
...
public class SynchronizeRegistrationUsernamePhase extends AbstractSynchronizeUsernamePhase<IdentityState> {
	...
	protected @NotNull String missingStateMessage() {
		return String.join("\n", messagesProvider.get().getScenarios().getRegistration().getErrors().getIdentity().getReplicationMissing());
	}
}
