	...
	protected Configura configura() {
		return Config.configured().withFeature(ExtensionFeature.rule(
				ConfigDocumentRule.when(
						Providers.ProviderEntry.Capabilities.class,
						JoinRestrictionCapabilities.class,
						JoinRestrictionProvidersProvider::isRestrictionCapabilitiesNode
				)
		));
	}

	private static boolean isRestrictionCapabilitiesNode(@NotNull DocumentTypeContext context) {
		return "capabilities".equals(context.getFieldName())
