...
public class Argon2CryptographyAlgorithm implements CryptographyAlgorithm {
	...
	public @NotNull String hash(@NotNull String password, @NotNull CryptographyOptions options) {
		...
		if (iterations <= 0) throw new IllegalArgumentException("argon2 iterations must be positive");
		if (memoryKb <= 0) throw new IllegalArgumentException("argon2 memoryKb must be positive");
		if (parallelism <= 0) throw new IllegalArgumentException("argon2 parallelism must be positive");
