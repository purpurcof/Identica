...
@Singleton
public class PremiumVelocityHandshakeApplierLifecycle implements EventListener {
	private final HandshakeApplierRegistry<VelocityHandshakeContext> applierRegistry;
	private final PremiumHandshakeApplier applier;

	@Inject
	public PremiumVelocityHandshakeApplierLifecycle(
			EventManager eventManager,
			HandshakeApplierRegistry<VelocityHandshakeContext> applierRegistry,
	...
	private boolean isPremium(InternalProvider provider) {
		...
		return provider
				.getDescriptor()
				.getId()
				.equalsIgnoreCase(PremiumConstants.PROVIDER_ID);
	}
}
